...
"""
完整对比实验
对比4组：API基线(零样本/CoT) vs LoRA微调(零样本/CoT)
"""
import os
import sys
import argparse
...
# 本地LoRA配置
LOCAL_CONFIG = {
    "model_path": "/home/zhayi/.cache/modelscope/hub/Qwen/Qwen2___5-7B-Instruct",
...
def print_final_comparison(all_results):
    ...
    print(f"{'API基线-CoT':<25} {api_cot['exact_match']:>11.2%} "
    ...
    print(f"{'LoRA微调-CoT':<25} {lora_cot['exact_match']:>11.2%} "
    ...
    print(f"1. 微调效果（零样本对比）: "
          f"F1提升 {ft_improvement:+.4f} ({ft_improvement/api_zs['avg_f1']*100:+.1f}%)")
    
    # 2. CoT效果（API）
    cot_improvement_api = api_cot['avg_f1'] - api_zs['avg_f1']
    print(f"2. CoT效果（API基线）: "
          f"F1提升 {cot_improvement_api:+.4f} ({cot_improvement_api/api_zs['avg_f1']*100:+.1f}%)")
    
    # 3. CoT效果（LoRA）
    cot_improvement_lora = lora_cot['avg_f1'] - lora_zs['avg_f1']
    print(f"3. CoT效果（LoRA微调）: "
          f"F1提升 {cot_improvement_lora:+.4f} ({cot_improvement_lora/lora_zs['avg_f1']*100:+.1f}%)")
...
def main():
    ...
    print(f"\n下一步:")
    print(f"  1. 查看详细结果: ls -lh {args.output_dir}/*/")
    print(f"  2. 运行500条评测: python scripts/07_full_comparison.py --eval_file data/evaluation/eval_500.json")
